...
/*   Updated: 2024/03/21 11:59:35 by yachen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/RobotomyRequestForm.hpp"

RobotomyRequestForm::RobotomyRequestForm( const std::string nm ) : AForm( nm , 72 , 45)
{
...
void	RobotomyRequestForm::execute( const Bureaucrate& ref ) const
{
	if (ref.getGrade() <= this->getReqSignGrade() && ref.getGrade() <= this->getReqExeGrade())
		std::cout << GREEN << ref.getName() << " has been robotomized successfully 50% of the time. " << RESET << std::endl;
	else
